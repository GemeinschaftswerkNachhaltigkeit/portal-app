...
SELECT *
FROM (SELECT FLOOR((ROW_NUMBER() OVER()-1)/2)*2 AS cluster_number, 'ORGANISATION' AS result_type,
      ...
      GROUP BY o1.id
      UNION ALL
      SELECT (FLOOR((ROW_NUMBER() OVER()-1)/2)*2)+1 AS cluster_number, 'ACTIVITY' AS result_type,
