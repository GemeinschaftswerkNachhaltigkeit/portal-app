...
FROM ( SELECT floor(((row_number() OVER (ORDER BY o1.modified_at DESC, o1.created_at DESC) - 1) / 2)::double precision) * 3::double precision AS cluster_number,
            ...
            o1.name,
            o1.description,
            setweight(to_tsvector('german'::regconfig, o1.name::text), 'A'::"char") || setweight(to_tsvector('german'::regconfig, o1.description), 'B'::"char") AS name_description_tsvec,
       ...
       SELECT floor(((row_number() OVER (ORDER BY a1.modified_at DESC, a1.created_at DESC) - 1) / 2)::double precision) * 3::double precision + 1::double precision AS cluster_number,
           ...
           a1.name,
           a1.description,
           setweight(to_tsvector('german'::regconfig, a1.name::text), 'A'::"char") || setweight(to_tsvector('german'::regconfig, a1.description), 'B'::"char") AS name_description_tsvec,
