...
SELECT *
FROM (SELECT FLOOR((ROW_NUMBER() OVER(ORDER BY o1.modified_at desc, o1.created_at desc)-1)/2)*3 AS cluster_number,
             ...
             o1.initiator                                                   AS initiator,
             o1.project_sustainability_winner                               AS project_sustainability_winner,
             o1.impact_area,
             o1.location_coordinate,
             o1.location_url, o1.location_online, o1.private_location,
      ...
      GROUP BY o1.id
      UNION ALL
      SELECT (FLOOR((ROW_NUMBER() OVER(ORDER BY a1.modified_at desc, a1.created_at desc)-1)/2)*3)+1 AS cluster_number,
             ...
             NULL                                                               AS initiator,
             NULL                                                               AS project_sustainability_winner,
             a1.impact_area,
             coalesce(a1.location_coordinate, organisation.location_coordinate) AS location_coordinate,
             a1.location_url, a1.location_online, a1.private_location,
      ...
      WHERE activity_type != 'DAN'
      GROUP BY a1.id, organisation.location_coordinate
      UNION ALL
      SELECT (FLOOR((ROW_NUMBER() OVER(ORDER BY a1.modified_at desc, a1.created_at desc)-1)/2)*3)+2 AS cluster_number,
